...
public sealed class FourKSaver : IVideoSaver
{
    ...
    static void Log(string folder, string fileName, [CallerMemberName] string? method = null)
        => Console.WriteLine($"Calling method {method} from {typeof(FourKSaver).Name} with folder {folder} and filename {fileName}.");
}
